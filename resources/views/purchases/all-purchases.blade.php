                            ...
                            <tbody>
                                @foreach ($purchases as $purchase)
                                    <tr>
                                        ...
                                        <td class="w-40">
                                            <div class="flex items-center justify-center {{ $purchase->order_status == 'pending' || $purchase->order_status == 'declined' ? 'text-red-500' : 'text-success' }}">
                                                <i data-lucide="check-square" class="w-4 h-4 mr-2"></i>
                                                {{ $purchase->order_status }}
                                            </div>
                                        </td>
                                        <td class="flex justify-center items-center gap-5">
                                            <a class="btn btn-primary py-1 px-2" href="{{ route('receive', ['id' => $purchase->id]) }}">Received</a>
                                            <a class="btn btn-primary py-1 px-2" href="{{ route('declined', $purchase->id) }}">Purchase Declined</a>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
